...
/// The notch window's current height. Grows to fit the Checklist always-on panel
/// when that mode is enabled; otherwise the standard fixed window height.
@MainActor func currentWindowHeight() -> CGFloat {
    if Defaults[.checklistAlwaysOn] {
        return max(windowSize.height, CGFloat(Defaults[.checklistAlwaysOnHeight]) + shadowPadding)
    }
